    ...
    try {
        ...
        (async () => {
            try {
                ...
                const gdEnabled = isGDriveEnabled();
                console.log(`[GDrive] isGDriveEnabled: ${gdEnabled}`);
                if (!gdEnabled) {
                    console.log('[GDrive] Skipping upload — GDrive not enabled/configured');
                    return;
                }
                ...
                console.log(`[GDrive] Uploading DOCX to: ${gDrivePath}`);

                const docxResult = await uploadToGDrive(docxPath, gDrivePath, `${safeBasename}.docx`);
                if (docxResult.success) {
                    console.log(`[GDrive] DOCX uploaded OK: ${docxResult.path}`);
                } else {
                    console.error(`[GDrive] DOCX upload FAILED`);
                }
                if (hasPdf) {
                    ...
                    if (fs.existsSync(pdfLocalPath)) {
                        ...
                        if (pdfResult.success) console.log(`[GDrive] PDF uploaded OK: ${pdfResult.path}`);
                        else console.error(`[GDrive] PDF upload FAILED`);
                    }
                }
            } catch (gErr: any) { console.error('[GDrive] Background upload error:', gErr.message, gErr.stack?.substring(0, 500)); }
        })();
...
function buildPeralatanTableXml(items: any[], fontInfo: { font: string; sz: string } = { font: '', sz: '' }): string {
    ...
    // Grid widths (twips): No=550 NamaAlat=1800 Merk=1200 Kapasitas=1300 Jumlah=1000 Kondisi=1050 StatusMilik=1300 = 8200tw
    const grid = '<w:tblGrid><w:gridCol w:w="550"/><w:gridCol w:w="1800"/><w:gridCol w:w="1200"/><w:gridCol w:w="1300"/><w:gridCol w:w="1000"/><w:gridCol w:w="1050"/><w:gridCol w:w="1300"/></w:tblGrid>';

    const headerRow = '<w:tr>' +
        cell('No', 330, { bold: true, center: true }) +
        cell('Nama Alat', 1100, { bold: true, center: true }) +
        cell('Merk & Tipe', 730, { bold: true, center: true }) +
        cell('Kapasitas', 790, { bold: true, center: true }) +
        cell('Jumlah', 610, { bold: true, center: true }) +
        cell('Kondisi', 640, { bold: true, center: true }) +
        cell('Status Milik', 800, { bold: true, center: true }) +
        '</w:tr>';

    const dataRows = items.map((it, i) => {
        const merkTipe = [it.merk, it.type].filter(Boolean).join(' ');
        return '<w:tr>' +
            cell(String(i + 1), 330, { center: true }) +
            cell(it.nama || '', 1100) +
            cell(merkTipe, 730, { center: true }) +
            cell(it.kapasitas || '', 790, { center: true }) +
            cell(String(it.jumlah || ''), 610, { center: true }) +
            cell(it.kondisi || '', 640, { center: true }) +
            cell(it.statusKepemilikan || '', 800, { center: true }) +
            '</w:tr>';
    }).join('');
...
function buildPeralatanTenderTableXml(items: any[], fontInfo: { font: string; sz: string } = { font: '', sz: '' }): string {
    ...
    const C = [650, 1650, 1300, 1500, 1100, 1164, 1950, 1186];
    const grid = '<w:tblGrid>' + C.map(w => '<w:gridCol w:w="' + w + '"/>').join('') + '</w:tblGrid>';
