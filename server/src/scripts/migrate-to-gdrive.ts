...
async function migrate() {
    ...
    const fotos = await db.select().from(sarprasFoto).where(
        sql`${sarprasFoto.filePath} NOT LIKE 'gdrive://%'`
    );
    ...
    const kerusakanFiles = await db.select().from(formKerusakan).where(
        sql`${formKerusakan.filePath} IS NOT NULL AND ${formKerusakan.filePath} NOT LIKE 'gdrive://%'`
    );
    ...
    const sertifikats = await db.select().from(prestasi).where(
        sql`${prestasi.sertifikatPath} IS NOT NULL AND ${prestasi.sertifikatPath} NOT LIKE 'gdrive://%'`
    );
