...
async function serveFileFromPath(filePath, res) {
    const normalizedPath = filePath.replace(/\\/g, '/');
    // 1. Local file
    if (fs.existsSync(filePath)) {
...
app.get('/api/file/prestasi/:id', async (req, res) => {
    try {
        ...
        if (!result[0] || !result[0].sertifikatPath) {
            res.status(404).json({ error: 'File not found' });
            return;
        }
...
app.get('/api/queue/status', async (_req, res) => {
    try {
        ...
        const count = async (table, col) => {
            const uploading = await database.select({ count: sql `count(*)` }).from(table).where(eq(table.uploadStatus, 'uploading'));
            const failed = await database.select({ count: sql `count(*)` }).from(table).where(eq(table.uploadStatus, 'failed'));
        ...
        res.json({ totalUploading: 0, totalFailed: 0, error: e.message });
    }
});
...
app.post('/api/queue/requeue', async (_req, res) => {
    try {
        ...
        let count = 0;
        const requeue = async (table, pathCol) => {
...
app.post('/api/npsn-login', async (req, res) => {
    try {
        ...
        if (!users.length)
            users = await db.select().from(user).where(eq(user.email, email2));
        // Fallback: search by NPSN prefix in email
        if (!users.length)
            users = await db.select().from(user).where(sql `"email" LIKE ${npsn + '@%'}`);
...
app.post('/api/admin/reset-sekolah-passwords', requireAuth, requireRole('admin'), async (_req, res) => {
    try {
...
app.post('/api/admin/fix-sekolah-links', requireAuth, requireRole('admin'), async (_req, res) => {
    try {
        const sekolahList = await db.select().from(sekolah);
        let fixed = 0;
        for (const sch of sekolahList) {
...
async function autoMigrate() {
    ...
    try {
        ...
        if (count > 0)
            console.log(`[AutoMigrate] Populated ${count} Sekolah passwords from NPSN`);
    }
    catch (_) { /* ignore */ }
}
app.listen(PORT, () => {
