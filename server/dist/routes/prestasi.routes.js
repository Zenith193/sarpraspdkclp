...
router.put('/:id', requireAuth, requireRole('admin', 'sekolah'), async (req, res) => {
    try {
        ...
        if (isSekolah) {
            ...
            const item = existing.data.find(d => d.prestasi.id === id);
            if (!item) {
...
router.post('/:id/verify', requireAuth, requireRole('admin', 'verifikator'), async (req, res) => {
    try {
        res.json(await prestasiService.verify(Number(req.params.id), req.user.id));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/:id/reject', requireAuth, requireRole('admin', 'verifikator'), async (req, res) => {
    try {
        res.json(await prestasiService.reject(Number(req.params.id), req.user.id, req.body.alasan));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/:id/unverify', requireAuth, requireRole('admin', 'verifikator'), async (req, res) => {
    try {
        res.json(await prestasiService.unverify(Number(req.params.id)));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.get('/rekap', requireAuth, requireRole('admin', 'korwil'), async (_req, res) => {
    try {
        res.json(await prestasiService.getRekap());
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.get('/point-rules', requireAuth, requireRole('admin'), async (_req, res) => {
    try {
        res.json(await prestasiService.listPointRules());
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/point-rules', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        res.status(201).json(await prestasiService.createPointRule(req.body));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.put('/point-rules/:id', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        res.json(await prestasiService.updatePointRule(Number(req.params.id), req.body));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.delete('/point-rules/:id', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        await prestasiService.deletePointRule(Number(req.params.id));
        res.json({ success: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
