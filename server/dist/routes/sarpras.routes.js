 ...
 */
function buildGDriveSarprasPath(kecamatan, namaSekolah, npsn, masaBangunan, namaRuang) {
 ...
 */
async function syncSarprasFolder(existing, oldMasa, oldRuang, newMasa, newRuang, masaChanged, ruangChanged) {
    ...
    try {
        ...
        if (oldMasa)
            oldLocalPath = path.join(oldLocalPath, sanitize(oldMasa));
...
router.post('/batch-by-npsn', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        ...
        items.forEach((item) => { if (item.npsn)
            npsnSet.add(String(item.npsn).trim()); });
        ...
        allSekolah.forEach(s => { if (s.npsn)
            npsnMap.set(s.npsn.trim(), s.id); });
        const validRows = [];
        const skippedNpsn = new Set();
        let skippedCount = 0;
...
router.post('/', requireAuth, requireRole('admin', 'sekolah'), uploadFotos.array('fotos', 5), forwardToNas('sarpras'), async (req, res) => {
    try {
        ...
        if (req.body.namaRuang)
            req.body.namaRuang = req.body.namaRuang.replace(/\//g, '');
    ...
    try {
        ...
        if (existing && existing.sekolahNama && existing.sekolahNpsn && existing.sekolahKecamatan) {
            ...
            if (masaChanged || ruangChanged) {
                syncSarprasFolder(existing, oldMasa, oldRuang, newMasa, newRuang, masaChanged, ruangChanged).catch(err => console.error('[SarprasSync] Error:', err.message));
            }
        }
    }
    catch (e) {
