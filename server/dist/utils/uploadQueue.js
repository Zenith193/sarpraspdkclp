    ...
    for (const item of pendingKerusakan) {
        ...
        try {
            ...
            if (item.sekolahId) {
                ...
                if (sch[0])
                    subPath = `${sch[0].kecamatan || 'unknown'}/${sch[0].nama}_${sch[0].npsn}/kerusakan`;
            }
    ...
    for (const item of pendingPrestasi) {
        ...
        try {
            ...
            if (item.sekolahId) {
                const sch = await db.select().from(sekolah).where(eq(sekolah.id, item.sekolahId));
                if (sch[0])
                    subPath = `${sch[0].kecamatan || 'unknown'}/${sch[0].nama}_${sch[0].npsn}/prestasi`;
            }
    ...
    for (const item of pendingProposal) {
        ...
        try {
            ...
            if (item.sekolahId) {
                const sch = await db.select().from(sekolah).where(eq(sekolah.id, item.sekolahId));
                if (sch[0])
                    subPath = `${sch[0].kecamatan || 'unknown'}/${sch[0].nama}_${sch[0].npsn}/proposal`;
            }
