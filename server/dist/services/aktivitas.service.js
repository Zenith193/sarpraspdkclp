...
export const aktivitasService = {
    async list(filters) {
        ...
        const conditions = [];
        if (jenisAkun)
            conditions.push(eq(aktivitas.jenisAkun, jenisAkun));
        if (from)
            conditions.push(gte(aktivitas.createdAt, new Date(from)));
        if (to)
            conditions.push(lte(aktivitas.createdAt, new Date(to)));
        ...
        const countResult = await db.select({ count: sql `count(*)` }).from(aktivitas).where(where);
