...
export const sarprasService = {
    async list(filters) {
        ...
        const conditions = [];
        if (sekolahId)
            conditions.push(eq(sarpras.sekolahId, sekolahId));
        if (kecamatan)
            conditions.push(eq(sekolah.kecamatan, kecamatan));
        if (jenjang)
            conditions.push(eq(sekolah.jenjang, jenjang));
        if (kondisi)
            conditions.push(eq(sarpras.kondisi, kondisi));
        if (verified === 'true')
            conditions.push(eq(sarpras.verified, true));
        if (verified === 'false')
            conditions.push(eq(sarpras.verified, false));
        if (search)
            conditions.push(ilike(sarpras.namaRuang, `%${search}%`));
        ...
        const [data, countResult] = await Promise.all([
            db.select({
                ...
                .limit(limit)
                .offset(offset),
            db.select({ count: sql `count(*)` }).from(sarpras).leftJoin(sekolah, eq(sarpras.sekolahId, sekolah.id)).where(where),
        ]);
