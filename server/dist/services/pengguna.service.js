...
export const penggunaService = {
    async list(filters) {
        ...
        try {
            data = await db.select({
                ...
                plainPassword: sql `"user"."plain_password"`.as('plain_password'),
                ...
                nip: sql `COALESCE(${sekolah.nip}, ${user.nip})`.as('nip'),
        ...
        const countResult = await db.select({ count: sql `count(*)` }).from(user).where(where);
    ...
    async update(id, data) {
        ...
        if (aktif !== undefined)
            userUpdate.aktif = aktif;
        // Save NIP on user table too (for non-sekolah accounts)
        if (sekolahData.nip !== undefined)
            userUpdate.nip = sekolahData.nip;
        ...
        if (updatedUser?.sekolahId) {
            const sekolahUpdate = { updatedAt: new Date() };
            if (sekolahData.jenjang !== undefined)
                sekolahUpdate.jenjang = sekolahData.jenjang;
            if (sekolahData.kecamatan !== undefined)
                sekolahUpdate.kecamatan = sekolahData.kecamatan;
            if (sekolahData.alamat !== undefined)
                sekolahUpdate.alamat = sekolahData.alamat;
            if (sekolahData.kepsek !== undefined)
                sekolahUpdate.kepsek = sekolahData.kepsek;
            if (sekolahData.nip !== undefined)
                sekolahUpdate.nip = sekolahData.nip;
            if (sekolahData.noRek !== undefined)
                sekolahUpdate.noRek = sekolahData.noRek;
            if (sekolahData.namaBank !== undefined)
                sekolahUpdate.namaBank = sekolahData.namaBank;
            if (sekolahData.rombel !== undefined)
                sekolahUpdate.rombel = sekolahData.rombel;
            if (name !== undefined)
                sekolahUpdate.nama = name; // sync sekolah name too
